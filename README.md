...
- **文件上传**：对话框支持上传文件并随消息一起发送
  - 支持图片格式：`.jpg` / `.jpeg` / `.png` / `.gif` / `.bmp`（消息中预览缩略图）
  - 支持文档格式：`.txt` / `.pdf` / `.doc` / `.docx` / `.ppt` / `.pptx` / `.xls` / `.xlsx`（提取文本内容发送给 AI）
  - 支持拖拽上传或点击选择，单次可附加多个文件
  - 单文件大小限制 20 MB，发送前可预览并移除已选文件
